$(document).ready(function () {
    ...
    var floorPath =  $('.floors path'); // каждый отдельный этаж в SVG
    ...
    modalCloseButton.on('click', toggleModal); // при клике на кнопку Закрыть - закыть окно
    viewFlatsButton.on('click', toggleModal);
 // отслеживаем клик по кнопке вверх
 counterUp.on('click', function() { 
    // проверяем знаяение этажа, не должно быть > 18
    if (currentFloor < 18) { 
        ...
        $('.counter').text(usCurrentFloor); // записываем знасение этажа в счетчик справа
        floorPath.removeClass('current-floor'); // удаляем активный класс у этажей
